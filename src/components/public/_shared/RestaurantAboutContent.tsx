/**
 * Conteúdo reutilizável "Sobre o restaurante": logo, nome, contato, horário, reserva, descrição.
...
export interface RestaurantAboutContentProps {
  restaurant: Restaurant;
  /** pt | es para textos e horários */
  lang: 'pt' | 'es';
  /** Base path para links (ex: `/${restaurant.slug}`) */
  basePath: string;
  /** Se fornecido, o botão "Fazer reserva" chama isso (ex: fechar modal) em vez de navegar */
  onReservaClick?: () => void;
}

export function RestaurantAboutContent({
  restaurant,
  lang,
  basePath,
  onReservaClick,
}: RestaurantAboutContentProps) {
  ...
  const fullWhatsApp = whatsapp
    ? (whatsapp.startsWith('55') || whatsapp.startsWith('595') || whatsapp.startsWith('54')
        ? whatsapp
        : prefix.replace('+', '') + whatsapp)
    : '';
  ...
  const instagramHref = instagramRaw?.startsWith('http')
    ? instagramRaw
    : instagramRaw
      ? `https://instagram.com/${instagramRaw.replace(/^@/, '')}`
      : null;
  const instagramHandle = instagramRaw
    ? instagramRaw.replace(/^https?:\/\/(www\.)?instagram\.com\/?/i, '').replace(/^@/, '').replace(/\/$/, '') || instagramRaw
    : null;
