...
interface ComparisonRow { name: string; basic: boolean; pro: boolean; enterprise: boolean }
...
export default function Pricing() {
  ...
  const sectionTitle    = mlc(c, 'main_pricing', 'section_title',    'Investimento que se paga com mais pedidos.');
  const sectionSubtitle = mlc(c, 'main_pricing', 'section_subtitle', 'Um preço fixo. Todas as funções do sistema. Sem surpresas na fatura.');
  const priceBasic      = mlc(c, 'main_pricing', 'price_basic',      '$15');
  const pricePro        = mlc(c, 'main_pricing', 'price_pro',        '$100');
  const priceEnterprise = mlc(c, 'main_pricing', 'price_enterprise', '$70+');
  const plans           = mlcJson<Plan[]>(c, 'main_pricing', 'plans', DEFAULT_PLANS);
  const comparison      = mlcJson<ComparisonRow[]>(c, 'main_pricing', 'comparison_features', DEFAULT_COMPARISON);

  const planPrices = { basic: priceBasic, pro: pricePro, enterprise: priceEnterprise };

  return (
    <section id="pricing" className="py-20 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full blur-3xl pointer-events-none opacity-20" style={{ backgroundColor: primaryColor }} />
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {plans.map((plan, index) => (
            ...
            >
              <Card
                ...
                style={plan.popular ? { borderColor: primaryColor } : {}}
              >
                {plan.popular && (
                  <div
                    ...
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-slate-600">
                        <Check
                          ...
                          style={{ color: plan.popular ? primaryColor : '#94a3b8' }}
                        />
                        <span>{feature}</span>
                      </li>
                    ))}
        ...
        >
          <h3 className="text-xl font-bold text-slate-900 text-center mb-6">Compare os planos</h3>
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="text-left py-4 px-4 font-semibold text-slate-900">Funcionalidade</th>
                  <th className="text-center py-4 px-4 font-semibold text-slate-600 w-[22%]">Basic</th>
                  <th className="text-center py-4 px-4 font-semibold w-[22%]" style={{ color: primaryColor }}>Pro</th>
                  <th className="text-center py-4 px-4 font-semibold text-slate-600 w-[22%]">Enterprise</th>
                </tr>
              </thead>
              <tbody>
                {comparison.map((row, i) => (
                  <tr key={i} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center">
                      {row.basic  ? <Check className="h-5 w-5 text-emerald-500 mx-auto" /> : <X className="h-5 w-5 text-slate-300 mx-auto" />}
                    </td>
                    <td className="py-3 px-4 text-center bg-orange-50/30">
                      {row.pro    ? <Check className="h-5 w-5 mx-auto" style={{ color: primaryColor }} /> : <X className="h-5 w-5 text-slate-300 mx-auto" />}
                    </td>
                    <td className="py-3 px-4 text-center">
                      {row.enterprise ? <Check className="h-5 w-5 text-emerald-500 mx-auto" /> : <X className="h-5 w-5 text-slate-300 mx-auto" />}
                    </td>
                  </tr>
                ))}
                <tr className="bg-slate-50 font-semibold">
                  ...
                  <td className="py-4 px-4 text-center bg-orange-50/30" style={{ color: primaryColor }}>{planPrices.pro}</td>
