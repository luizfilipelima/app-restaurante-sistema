-- Inclui addon_mode, addon_min, addon_max, addon_required no productAddonsMap
-- (usa estrutura existente do 20260519, alterando apenas o jsonb_build_object dos grupos)
...
DECLARE
  ...
  v_result       jsonb;
BEGIN
  SELECT id, to_jsonb(r.*) INTO v_rid, v_restaurant FROM restaurants r WHERE r.slug = p_slug AND r.is_active = true;
  IF v_rid IS NULL THEN RETURN NULL; END IF;

  SELECT COALESCE(jsonb_agg(row_to_json(c.*) ORDER BY c.order_index), '[]'::jsonb) INTO v_categories FROM categories c WHERE c.restaurant_id = v_rid;
  SELECT COALESCE(jsonb_agg(row_to_json(s.*) ORDER BY s.order_index), '[]'::jsonb) INTO v_subcats FROM subcategories s WHERE s.restaurant_id = v_rid;
  SELECT COALESCE(jsonb_agg(row_to_json(p.*) ORDER BY COALESCE(cat.order_index, 999), p.order_index), '[]'::jsonb) INTO v_products
  FROM products p LEFT JOIN categories cat ON cat.name = p.category AND cat.restaurant_id = v_rid
  ...
  SELECT COALESCE((SELECT jsonb_object_agg(combo_product_id, items) FROM (
    SELECT pci.combo_product_id,
      ...
      ) ORDER BY pci.sort_order) AS items
  ...
  ) sub), '{}'::jsonb) INTO v_combo_map;

  SELECT COALESCE((SELECT jsonb_object_agg(ag_data.product_id, ag_data.groups) FROM (
    SELECT ag.product_id,
      jsonb_agg(jsonb_build_object(
        'id', ag.id, 'product_id', ag.product_id, 'name', ag.name, 'order_index', ag.order_index,
        'addon_mode', COALESCE(ag.addon_mode, 'quantidade'),
        'addon_min', COALESCE(ag.addon_min, 0),
        'addon_max', COALESCE(ag.addon_max, 5),
        'addon_required', COALESCE(ag.addon_required, false),
        'items', COALESCE(ai_agg.items, '[]'::jsonb)
      ) ORDER BY ag.order_index) AS groups
    FROM product_addon_groups ag
    LEFT JOIN LATERAL (SELECT jsonb_agg(row_to_json(ai.*) ORDER BY ai.order_index) AS items FROM product_addon_items ai WHERE ai.addon_group_id = ag.id) ai_agg ON true
    WHERE ag.product_id IN (SELECT id FROM products WHERE restaurant_id = v_rid)
    GROUP BY ag.product_id
  ) ag_data), '{}'::jsonb) INTO v_addon_map;

  SELECT COALESCE(jsonb_agg(cat_name ORDER BY min_order), '[]'::jsonb) INTO v_cats_arr FROM (
  ...
  v_result := jsonb_build_object(
    'restaurant', v_restaurant, 'products', v_products, 'categories', v_cats_arr, 'categoriesFromDb', v_categories, 'subcategories', v_subcats,
    'pizzaSizes', v_pizza_sizes, 'pizzaFlavors', v_pizza_flav, 'pizzaDoughs', v_pizza_dough, 'pizzaEdges', v_pizza_edge, 'pizzaExtras', COALESCE(v_pizza_extras, '[]'::jsonb),
    'marmitaSizes', v_marm_sizes, 'marmitaProteins', v_marm_prot, 'marmitaSides', v_marm_sides,
    'productComboItemsMap', COALESCE(v_combo_map, '{}'::jsonb), 'productAddonsMap', COALESCE(v_addon_map, '{}'::jsonb)
  );
  RETURN v_result;
END;
$$;
