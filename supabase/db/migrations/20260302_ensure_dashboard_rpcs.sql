...
-- 2. Recriar get_dashboard_analytics (conteúdo idêntico a create_dashboard_analytics_rpc.sql)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'sales_trend', COALESCE((
      WITH date_series AS (
        SELECT generate_series(date_trunc('day', p_start_date)::DATE, date_trunc('day', p_end_date)::DATE, '1 day'::INTERVAL)::DATE AS day),
      daily_sales AS (
...
-- 3. Recarrega o schema do PostgREST (evita PGRST202 / 400 por cache desatualizado)
