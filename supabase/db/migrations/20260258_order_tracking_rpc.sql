-- Migration: get_order_tracking RPC
-- Permite que clientes anônimos rastreiem um pedido pelo ID sem autenticação.
-- Retorna apenas os campos necessários para a tela de tracking pública.
-- A função SECURITY DEFINER bypassa RLS de forma segura, pois o acesso
-- é restrito por order_id (UUID não-adivinhável).
...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    'courier', CASE
      WHEN v_order.courier_id IS NOT NULL AND v_courier.id IS NOT NULL THEN
        jsonb_build_object(
          'id',            v_courier.id,
          'name',          v_courier.name,
